...
let%expect_test "Cyclesim with single clock port" =
  List.iter Bool.all ~f:(fun specify_clock_names ->
    ...
    let sim =
      Hardcaml_verilator.create
        ?clock_names:(if specify_clock_names then Some [ "clk" ] else None)
        circuit
    in
    let test () =
      let inputs = Cyclesim.inputs sim in
      let find name =
        List.find_exn inputs ~f:(fun a -> String.equal name (fst a)) |> snd
      in
