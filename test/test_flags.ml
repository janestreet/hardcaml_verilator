...
let%expect_test "labels" =
  ...
  [%expect {| O3-procs1-threads1-perfile0-perfunc0 |}];
  Stdio.print_string Config.(label small_cfiles);
  [%expect {| O3-procs1-threads1-perfile5000-perfunc500 |}]
;;
...
let%expect_test "default - v5" =
  ...
  [%expect
    {|
    CXXFLAGS="-fPIC" verilator -O3 --threads 1 --top-module circuit   -Wno-COMBDLY -Wno-CMPCONST -Wno-UNSIGNED -Wno-INITIALDLY -Wno-MULTIDRIVEN --cc verilog  --Mdir obj_dir wrapper
...
let%expect_test "small_cfiles - v5" =
  ...
  [%expect
    {|
    CXXFLAGS="-fPIC" verilator -O3 --threads 1 --top-module circuit --output-split  5000 --output-split-cfuncs  500 --output-split-ctrace 500  -Wno-COMBDLY -Wno-CMPCONST -Wno-UNSIGNED -Wno-INITIALDLY -Wno-MULTIDRIVEN --cc verilog  --Mdir obj_dir wrapper
    CXXFLAGS="-fPIC -g -O3" OBJCACHE=""  make  -C obj_dir -f Vcircuit.mk Vcircuit__ALL.a wrapper.o verilated.o verilated_threads.o
    g++ -latomic -lpthread -fPIC -O3 -g -shared -o shared obj_dir/wrapper.o obj_dir/verilated.o obj_dir/verilated_threads.o static
    |}]
;;
